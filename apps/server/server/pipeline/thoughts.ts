...
const thoughtTemplates: Record<string, string[] | Record<string, string[]>> = {
  turn_started: [
 ...
 */
export function generateThought(step: string, context: any = {}): string {
  const ctx = {
  ...
  if (step === "intent_parsed") {
    const pool = (thoughtTemplates.intent_parsed as Record<string, string[]>)[ctx.intentType];
    if (pool) {
      return interpolate(pickRandom(pool), ctx);
    }
  }

  if (step === "skill_selected") {
    const pool = (thoughtTemplates.skill_selected as Record<string, string[]>)[ctx.skill];
    if (pool) {
      return interpolate(pickRandom(pool), ctx);
    }
  }
  ...
  if (thoughtTemplates[step]) {
    const templateData = thoughtTemplates[step];
    const pool = Array.isArray(templateData)
      ? templateData
      : Object.values(templateData).flat();
    return interpolate(pickRandom(pool), ctx);
  }
 ...
 */
export function tokenizeThought(thought: string): string[] {
