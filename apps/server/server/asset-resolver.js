...
const CURATED_THREEJS_ASSETS = Object.freeze({
...
function collectSourceText({ sourceText = "", parsedIntent = null } = {}) {
  const parts = [];
...
export function resolveAssetCategories(sourceText) {
...
export function buildAssetPolicyText(assetPlan) {
...
export function buildAssetCatalogText(assetPlan) {
  if (!assetPlan || assetPlan.skill !== "threejs") {
...
export function buildQualityContractText(assetPlan) {
  if (!assetPlan || assetPlan.skill !== "threejs") {
    return "N/A for non-Three.js skills.";
  }
...
export function getCuratedThreejsAssetCatalog() {
