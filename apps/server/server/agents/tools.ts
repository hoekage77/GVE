...
const runtimeExecutionTimeoutMs = parsePositiveIntEnv(process.env.RUNTIME_EXEC_TIMEOUT_MS, 2200, 300);
const manimRuntimeExecutionTimeoutMs = parsePositiveIntEnv(process.env.RUNTIME_EXEC_TIMEOUT_MANIM_MS, 90_000, 2_000);
const runtimeExecutionMaxFrames = parsePositiveIntEnv(process.env.RUNTIME_EXEC_MAX_FRAMES, 48, 1);
...
function getRemainingBudgetMs(deadlineAtMs?: number | null): number {
  if (!deadlineAtMs || !Number.isFinite(deadlineAtMs)) {
    return Number.POSITIVE_INFINITY;
  }
  return Math.max(0, deadlineAtMs - Date.now());
}

function computeBoundedTimeoutMs(deadlineAtMs: number | undefined | null, configuredTimeoutMs: number, minimumTimeoutMs = 300): number {
  ...
  validate_code: {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: {
          code: { type: "string", description: "The scene code to validate." },
          skill: {
            type: "string",
  ...
  fix_code: {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: {
          code: { type: "string", description: "The corrected scene code." },
          skill: {
            type: "string",
            enum: ["threejs", "p5js", "d3js", "animejs", "manim"],
            description: "The rendering skill/engine the code targets."
          },
          changes_made: { type: "string", description: "Brief description of what was fixed." }
        },
        required: ["code", "skill"]
      }
    }
  ...
  execute_code: {
    ...
    function: {
      ...
      parameters: {
        type: "object",
        properties: {
          code: { type: "string", description: "The validated scene code to execute." },
          skill: {
            type: "string",
            enum: ["threejs", "p5js", "d3js", "animejs", "manim"],
...
async function handleValidateCode({ code, skill }: { code: string; skill?: string }) {
  const result = await validateCode(code, skill ?? "threejs");
  return {
    valid: result.valid,
    ...
    summary: result.valid
      ...
      : `Found ${result.errors?.length || 0} issue(s): ${(result.errors || []).map((e: any) => e.message || String(e)).join("; ")}`
  };
}

async function handleFixCode({ code, skill, changes_made }: { code: string; skill?: string; changes_made?: string }) {
  const result = await validateCode(code, skill ?? "threejs");
  return {
    valid: result.valid,
    passable: result.passable,
    ...
    summary: result.valid
      ...
      : `Still has ${result.errors?.length || 0} issue(s): ${(result.errors || []).map((e: any) => e.message || String(e)).join("; ")}`
  };
}

async function handleExecuteCode({ code, skill }: { code: string; skill?: string }) {
  try {
...
function handleGetErrorContext({ include_code }: { include_code?: boolean }) {
