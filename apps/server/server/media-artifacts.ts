/**
 * Media Artifacts Manager
 *
 * Handles saving and serving generated media (videos, images) from the sandbox.
 */
...
function normalizeExtension(extension: string | null | undefined, fallback = "mp4"): string {
...
function sanitizeMediaKey(mediaKey: string | null | undefined): string | null {
  const normalized = basename(String(mediaKey ?? "").trim());
  if (!normalized) return null;
  if (!/^[a-zA-Z0-9._-]+$/.test(normalized)) return null;

  const extension = extname(normalized).replace(/^\./, "").toLowerCase();
  if (!extension || !MIME_BY_EXTENSION[extension]) return null;

  return normalized;
}

function parseByteRange(rangeHeader: string | null | undefined, totalSize: number): { start: number; end: number } | null {
  ...
  let start: number | null = startRaw ? Number.parseInt(startRaw, 10) : null;
  let end: number | null = endRaw ? Number.parseInt(endRaw, 10) : null;

  if (start === null && end === null) return null;

  if (start === null) {
    const suffixLength = typeof end === "number" && Number.isFinite(end) ? end : 0;
...
function deriveContentType(mediaKey: string, explicitType?: string | null): string {
...
export function buildMediaPreviewUrl(mediaKey: string): string {
  return `/api/v1/media/${mediaKey}`;
}

export interface PersistMediaArtifactOptions {
  buffer: Buffer;
  extension?: string;
...
export function persistMediaArtifact({
  ...
  sessionId = null,
  sceneId = null
}: PersistMediaArtifactOptions): { mediaKey: string; mediaType: string; sizeBytes: number; previewUrl: string } {
...
export function streamMediaArtifact(req: any, res: any, mediaKey: string): void {
  const safeMediaKey = sanitizeMediaKey(mediaKey);
  if (!safeMediaKey) {
    res.status(400).json({ error: "VALIDATION_ERROR", message: "Invalid media key." });
    return;
  }

  const absolutePath = join(MEDIA_DIR, safeMediaKey);
  if (!existsSync(absolutePath)) {
    res.status(404).json({ error: "NOT_FOUND", message: "Media artifact not found." });
    return;
  }
  ...
  if (rangeHeader) {
    ...
    if (!byteRange) {
      res.status(416).set({ "Content-Range": `bytes */${totalSize}`, "Accept-Ranges": "bytes" }).end();
      return;
    }
