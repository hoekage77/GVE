// @ts-nocheck
import { metrics, recordLatency } from "../lib/metrics.js";
import { broadcastThought, broadcastCodeStream, broadcastEvent } from "../ws/streaming.js";
import { 
  createSession, createSessionMessageId, appendSessionMessage, updateSessionMessage, 
  appendOrchestrationTrace, setSessionStatus, listSessionMessages, recordSceneVersion, 
  buildSceneUpdatePayload, 
  buildCodeUpdatePayload, undoSceneVersion, redoSceneVersion, previousRevision, 
  nextRevision, previousSceneVersion, nextSceneVersion, previousArtifactVersion, 
  nextArtifactVersion 
} from "../session-state.js";
import { 
  resolveChatTurn, generateThinkingAnalysis, planTasks, generateFromImage, generatePostTurnNarration 
...
export function normalizeRequestedTurnMode(rawMode) {
  const normalized = String(rawMode ?? "").trim().toLowerCase();
  if (normalized === "modify" || normalized === "generate") return normalized;
  return null;
}

export function normalizeSceneCommand(rawCommand) {
...
export function normalizeTurnPreferences(preferences, forcedMode = null) {
  const basePreferences = preferences && typeof preferences === "object" && !Array.isArray(preferences) ? preferences : {};
  if (!forcedMode) return basePreferences;
  return { ...basePreferences, mode: forcedMode };
}
export async function executeChatTurn(sessionId, content, preferences, options = {}) {
...
export function executeSceneCommandMutation(sessionId, command) {
...
export function buildAgentActivity({ sessionId, messageId, step, status = "running", payload = {} }) {
...
export function buildTurnLifecyclePayload(turnSummary) {
  return {
