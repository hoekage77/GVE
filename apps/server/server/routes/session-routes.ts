...
sessionRouter.get("/api/v1/sessions", requireAuthOrApiKey, async (req, res) => {
  try {
    const userId = resolveUserId(req);
    ...
    const result = await sessionRepo.findByOwner(userId!, { limit, offset, archived });
    const sessions = result.data.map((row) => {
      const inMemory = getSessionState(row.id);
      return {
        sessionId: row.id,
        name: row.title,
        status: row.status,
        ...
        currentScene: inMemory?.currentScene ?? null,
        versions: inMemory?.versions ?? [],
        versionCount: inMemory?.versionCount ?? 0,
        versionPointer: inMemory?.versionPointer ?? 0,
        sceneVersions: inMemory?.sceneVersions ?? [],
        artifacts: inMemory?.artifacts ?? [],
        artifactCount: inMemory?.artifactCount ?? 0,
        artifactPointer: inMemory?.artifactPointer ?? 0,
        currentArtifactId: inMemory?.currentArtifactId ?? null,
        canUndo: inMemory?.canUndo ?? false,
        canRedo: inMemory?.canRedo ?? false,
        canPreviousArtifact: inMemory?.canPreviousArtifact ?? false,
        canNextArtifact: inMemory?.canNextArtifact ?? false,
        sceneId: inMemory?.sceneId ?? null,
      };
    });
...
sessionRouter.get("/api/v1/sessions/:sessionId", requireSessionOwnership, async (req, res) => {
  try {
...
sessionRouter.patch("/api/v1/sessions/:sessionId", requireSessionOwnership, async (req, res) => {
  try {
    const sessionId = String(req.params.sessionId);
...
sessionRouter.delete("/api/v1/sessions/:sessionId", requireSessionOwnership, async (req, res) => {
  try {
    const sessionId = String(req.params.sessionId);
...
sessionRouter.get("/api/v1/sessions/:sessionId/messages", requireSessionOwnership, async (req, res) => {
  try {
    const sessionId = String(req.params.sessionId);
    ...
    const result = await messageRepo.findBySession(sessionId, { limit, offset, before });
...
sessionRouter.patch("/api/v1/sessions/:sessionId/messages/:messageId", requireSessionOwnership, async (req, res) => {
  try {
    const messageId = String(req.params.messageId);
    const patch = req.body ?? {};
    const updated = await messageRepo.update(messageId, {
...
sessionRouter.delete("/api/v1/sessions/:sessionId/messages/:messageId", requireSessionOwnership, async (req, res) => {
  try {
    const messageId = String(req.params.messageId);
...
sessionRouter.post("/api/v1/sessions/:sessionId/modify", requireSessionOwnership, async (req, res) => {
  ...
  try {
    const result = await runWithTraceContext({ sessionId, requestId }, () =>
      executeChatTurn(sessionId, instruction, req.body?.preferences ?? {}, {
        requestId,
        clientMessageId,
        forcedMode: "modify",
        transport: "rest",
